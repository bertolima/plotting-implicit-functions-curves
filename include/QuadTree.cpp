...
QuadTree::QuadTree(float width, float height, float coodX, float coodY, int max_depth, int depth){
    ...
    this->rec.setOrigin(0,this->rec.getGlobalBounds().width);
...
void QuadTree::division(QuadTree* tree){
    ...
    this->norte_esq = new QuadTree(width, height, codX, codY-height, tree->getMax_depth(), tree->getDepth()+1);
    this->norte_dir = new QuadTree(width, height, codX+width, codY-height, tree->getMax_depth(), tree->getDepth()+1);
    this->sul_dir = new QuadTree(width, height, codX+width, codY, tree->getMax_depth(), tree->getDepth()+1);
    this->sul_esq = new QuadTree(width, height, codX, codY, tree->getMax_depth(), tree->getDepth()+1);
...
bool QuadTree::contains(QuadTree* tree){
    ...
    float esq = tree->coodX;
    float dir = tree->coodX + tree->getRec().getGlobalBounds().width/2;
    float cima = 512 + -1*(tree->coodY - tree->getRec().getGlobalBounds().height/2);
    float baixo = 512 + -1*(tree->coodY + tree->getRec().getGlobalBounds().height/2);
    ...
    float baixoDir = f1(this->ratio*(dir - tree->getRec().getGlobalBounds().width/2), this->ratio*(baixo - tree->getRec().getGlobalBounds().width/2));
    float baixoEsq = f1(this->ratio*(esq - tree->getRec().getGlobalBounds().width/2), this->ratio*(baixo - tree->getRec().getGlobalBounds().width/2));
